...
void clampedExpVector(float *values, int *exponents, float *output, int N)
{
  ...
  __cs149_mask maskIsNotZero;
  __cs149_vec_float result;
  if (N < VECTOR_WIDTH)
  {
    clampedExpSerial(values, exponents, output, N);
    return;
  }
  for (int i = 0; i < N; i += VECTOR_WIDTH)
  {
    ...
    _cs149_vgt_int(maskIsNotZero, y, zero, maskAll); // maskIsNotZero = (y > 0);
    ...
    _cs149_vgt_float(maskIsNotZero, result, clampValue, maskAll); // if (result > 9.999999f);
    _cs149_vset_float(result, 9.999999f, maskIsNotZero);          //   result = 9.999999f;
    _cs149_vstore_float(output + i, result, maskAll);             //   output[i] = result;
  }
  int res = N % VECTOR_WIDTH;
  if (res != 0)
    clampedExpSerial(values + N - res, exponents + N - res, output + N - res, res);
}
...
float arraySumVector(float *values, int N)
{
  ...
  for (int j = VECTOR_WIDTH; j > 2; j /= 2)
  {
